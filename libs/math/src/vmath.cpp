...
Polygon vmath::makeIntPolygon(Point &point, size_t size)
{
    ...
    for (int i = 1; i < size; ++i)
    {
        ...
        int_poly[i] = {round(reg_poly[i].GetX()), round(reg_poly[i].GetY())};
    }
    return int_poly;
}
...
{
    auto n = static_cast<double>(poly.size());
...
Polygon::Point vmath::firstFourierCoeff(Polygon &poly)
{
    ...
    auto n = static_cast<double>(n_int);

    Point Coeff1; // среднее арифметическое отклонений
    for (int i = 0; i < n_int; ++i)
    { // каждой вершины от "центра масс"
        Point temp = poly[i];
...
double vmath::calculateEpsilon(Polygon &poly)
{
    ...
    for (int i = 0; i < n_int; ++i)
    { // шагаем по соотв вершинам и считаем расстояние
